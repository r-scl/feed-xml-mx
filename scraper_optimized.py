...
from error_handling import NetworkError, ScrapingError, handle_async_errors
...
class OptimizedProductScraper:
    ...
    async def _scrape_with_retries(
        self, product_url: str, product_id: str, cache_key: str
    ) -> Optional[ScrapedProductData]:
        ...
        for attempt in range(self.config.retry_attempts + 1):
            try:
                ...
                if attempt < self.config.retry_attempts:
                    ...
                    logger.warning(
                        "Retrying after error", product_id=product_id, attempt=attempt + 1, delay=delay, error=str(e)
                    )
                    await asyncio.sleep(delay)
                else:
                    logger.error("Max retries exceeded", product_id=product_id, error=str(e))
    ...
    async def _perform_scrape(self, product_url: str, product_id: str) -> Optional[ScrapedProductData]:
        ...
        async with self._browser_session():
            ...
            try:
                ...
                if any(
                    error_text in page_title.lower()
                    for error_text in ["404", "not found", "página no encontrada", "error"]
                ):
                    raise ScrapingError("Product page not found (404)", product_id=product_id, url=product_url)
    ...
    async def _extract_price_info_async(self, soup: BeautifulSoup) -> Dict[str, Optional[Union[float, int, str]]]:
        ...
        return await asyncio.get_event_loop().run_in_executor(None, self._extract_price_info_sync, soup)
    ...
    async def _extract_stock_info_async(self, soup: BeautifulSoup) -> Optional[int]:
        ...
        return await asyncio.get_event_loop().run_in_executor(None, self._extract_stock_info_sync, soup)
    ...
    async def _extract_description_async(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        return await asyncio.get_event_loop().run_in_executor(None, self._extract_description_sync, soup)
    ...
    async def _extract_images_async(self, soup: BeautifulSoup) -> List[str]:
        ...
        return await asyncio.get_event_loop().run_in_executor(None, self._extract_images_sync, soup)
    ...
    async def scrape_multiple_products(self, product_urls: List[tuple]) -> Dict[str, ScrapedProductData]:
        ...
        tasks = [self.scrape_product_page(url, product_id) for url, product_id in product_urls]
        ...
        for i in range(0, len(tasks), batch_size):
            ...
            for j, result in enumerate(batch_results):
                ...
                if isinstance(result, Exception):
                    logger.error("Product scraping failed", product_id=product_id, error=str(result))
                elif result:
                ...
                if completed % 10 == 0:
                    ...
                    logger.info(
                        "Scraping progress", completed=completed, total=len(product_urls), progress=f"{progress:.1f}%"
                    )
    ...
    def clear_cache(self):
        ...
        logger.info("Cache cleanup completed", expired_entries=len(expired_keys), remaining_entries=len(self.cache))
...
def monitor_performance(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        try:
            ...
            logger.debug("Function completed", function=func.__name__, duration=f"{duration:.3f}s")
            return result
        except Exception as e:
            duration = time.time() - start_time
            logger.error("Function failed", function=func.__name__, duration=f"{duration:.3f}s", error=str(e))
            raise
