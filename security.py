...
from error_handling import ConfigurationError, ErrorSeverity, SecurityError
...
class RequestTracker:
    ...
    def _block_temporarily(self, identifier: str, duration: int):
        ...
        logger.warning("IP temporarily blocked for rate limiting", ip=identifier, duration=duration)
...
class URLValidator:
    ...
    def validate_url(self, url: str, allow_relative: bool = False) -> bool:
        ...
        try:
            ...
            if parsed.netloc:
                ...
                if self.config.validate_urls and not any(allowed in domain for allowed in self.config.allowed_domains):
    ...
    def sanitize_url(self, url: str) -> str:
        ...
        if not self.validate_url(url):
            raise SecurityError(
                "Invalid or dangerous URL detected", context={"url": url[:100]}, severity=ErrorSeverity.HIGH
            )
        ...
        try:
            ...
            raise SecurityError("URL sanitization failed", context={"url": url[:100]}, original_error=e)
...
class ContentSanitizer:
    ...
    def validate_file_content(self, content: bytes, filename: str) -> bool:
        ...
        if len(content) > self.config.max_file_size:
            raise SecurityError(
                f"File too large: {len(content)} bytes", context={"filename": filename, "size": len(content)}
            )

        # Check file extension
        _, ext = os.path.splitext(filename.lower())
        if ext not in self.config.allowed_file_extensions:
            raise SecurityError(f"File extension not allowed: {ext}", context={"filename": filename, "extension": ext})
        ...
        if ext in [".xml", ".json", ".txt"]:
            try:
                ...
                raise SecurityError("Binary content in text file", context={"filename": filename})
        ...
        for pattern in malicious_patterns:
            if pattern.decode("utf-8", errors="ignore") in content_str:
                raise SecurityError(
...
class SecurityManager:
    ...
    def validate_request(self, request_data: Dict[str, Any], client_ip: str = "unknown") -> bool:
        ...
        if self.request_tracker.is_rate_limited(client_ip, self.config):
            raise SecurityError("Rate limit exceeded", context={"client_ip": client_ip}, severity=ErrorSeverity.MEDIUM)

        # Validate request size
        request_size = len(str(request_data))
        if request_size > self.config.max_content_length:
            raise SecurityError(
                "Request too large", context={"size": request_size, "max_size": self.config.max_content_length}
            )
        ...
        for key, value in request_data.items():
            if isinstance(value, str) and ("url" in key.lower() or "link" in key.lower()):
                if not self.url_validator.validate_url(value):
                    raise SecurityError(f"Invalid URL in field {key}", context={"field": key, "url": value[:100]})
        ...
        for key, value in product_data.items():
            if isinstance(value, str):
                ...
                if isinstance(value, list):
                    sanitized[key] = [
                        self.content_sanitizer.sanitize_text(str(item))[:500] if isinstance(item, str) else item
                        for item in value[:10]  # Limit list size
                    ]
                else:
                    sanitized[key] = {
                        k: self.content_sanitizer.sanitize_text(str(v))[:200] if isinstance(v, str) else v
    ...
    def log_security_event(self, event_type: str, details: Dict[str, Any]):
        ...
        logger.warning(
            "Security event detected", event_type=event_type, timestamp=datetime.now().isoformat(), **details
        )
    ...
    def decorator(func):
        ...
        async def wrapper(*args, **kwargs):
            ...
            if security_manager.request_tracker.is_rate_limited(identifier, security_manager.config):
                raise SecurityError("Rate limit exceeded", context={"identifier": identifier})
...
class SecureConfig:
    ...
    def get_env_var(name: str, default: Optional[str] = None, required: bool = False) -> Optional[str]:
        ...
        if required and value is None:
            raise ConfigurationError(f"Required environment variable {name} not found", config_key=name)
