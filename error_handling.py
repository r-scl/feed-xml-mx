...
class ScrapingError(FeedXMLError):
    ...
    def __init__(self, message: str, product_id: Optional[str] = None, url: Optional[str] = None, **kwargs):
        ...
        context.update({"product_id": product_id, "url": url})
...
class ProcessingError(FeedXMLError):
    ...
    def __init__(self, message: str, feed_type: Optional[str] = None, product_count: Optional[int] = None, **kwargs):
        context = kwargs.pop("context", {})
        context.update({"feed_type": feed_type, "product_count": product_count})
...
class ErrorHandler:
    ...
    async def handle_error(
        self, error: Exception, context: Optional[Dict[str, Any]] = None, response_stream: Any = None
    ) -> None:
        """
        ...
        try:
            ...
            if not isinstance(error, FeedXMLError):
                error = FeedXMLError(str(error), original_error=error, context=context)
    ...
    async def _handle_critical_error(self, error: FeedXMLError) -> None:
        ...
        self.logger.critical("Critical error - system stability may be compromised", error_details=error.to_dict())
    ...
    async def _send_alerts_if_needed(self, error: FeedXMLError) -> None:
        ...
        if error.severity in (ErrorSeverity.HIGH, ErrorSeverity.CRITICAL):
            ...
            self.logger.warning("Alert: High/critical error requires attention", error_summary=error.to_dict())
...
def setup_global_error_handlers():
    ...
    def handle_exception(exc_type, exc_value, exc_traceback):
        ...
        error = FeedXMLError(
            ...
            context={"exception_type": exc_type.__name__, "traceback": "".join(traceback.format_tb(exc_traceback))},
        )
